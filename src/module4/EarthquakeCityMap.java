...
public class EarthquakeCityMap extends PApplet {
	...
	private int oceanQuakes;
	private int totalNumQuakes = 0;
	...
	private void addKey() {
		...
		fill(color(255, 0, 0));
		triangle(50, 125, 55, 120, 60, 125);
		fill(color(0, 0, 0));
		text("City Marker", 75, 120);
		/*
		 * fill(color(255, 0, 0)); ellipse(50, 125, 15, 15); fill(color(255,
		 * 255, 0)); ellipse(50, 175, 10, 10); fill(color(0, 0, 255));
		 * ellipse(50, 225, 5, 5); fill(0, 0, 0); text("5.0+ Magnitude", 75,
		 * 125); text("4.0+ Magnitude", 75, 175); text("Below 4.0", 75, 225);
		 */
	}
	...
	private void printQuakes() {
		...
		for (Marker cm : countryMarkers) {
			...
			for (PointFeature earthquake : earthquakes) {
				...
				if (isInCountry(earthquake, cm)) {
					numQuakes++;
					country = (String) cm.getProperty("name");
				} else {
					// OceanQuakes
					// new OceanQuakeMarker(earthquake);
					oceanQuakes++;
				}
			}
			...
			if (numQuakes > 0) {
				...
				totalNumQuakes += numQuakes;
			}

		}
		System.out.println("OCEANQUAKES" + ":" + (oceanQuakes - totalNumQuakes));
	}

	// Then loop through all of the earthquake
	// markers and check to see whether
	// (1) that marker is on land
	// and (2) if it is on land,
	// that its country property
	// matches the name property of the
	// country marker. If so, increment; the country's counter.
	// String country = (String)earthquake.getProperty("country");
	// OceanQuakes // else { // new OceanQuakeMarker(earthquake);
